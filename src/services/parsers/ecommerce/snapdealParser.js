// src/services/parsers/snapdealParser.js
...
class SnapdealParser {
  ...
  static canParse(emailData) {
    ...
    const isSnapdeal =
      from.includes("snapdeal.com") || from.includes("@snapdeal");
    if (!isSnapdeal) return false;
  ...
  parse(emailData) {
    try {
      const cleanContent = this.cleanSnapdealHtml(
  ...
  extractOrderId(content, subject) {
    const patterns = [
      /Order\s*(?:ID|Number|#)\s*[:\-]?\s*(SD[A-Z0-9]{8,15})/gi,
  ...
  isValidProduct(name) {
    ...
    const invalidPatterns = [/order|total|amount|snapdeal|email|notification/i];
