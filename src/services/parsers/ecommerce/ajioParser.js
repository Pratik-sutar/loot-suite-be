// src/services/parsers/ajioParser.js
...
class AjioParser {
  ...
  /**
   * Enhanced canParse method with comprehensive AJIO sender detection
   */
  static canParse(emailData) {
    console.log("🛍️ AJIO DEBUG: canParse() called");
    console.log(`📧 From: ${emailData.from}`);
    console.log(`📋 Subject: ${emailData.subject}`);

    if (!emailData.from || !emailData.subject) {
      console.log("❌ AJIO: Missing from/subject");
      return false;
    }
    ...
    // Complete AJIO domain variations
    const ajioDomains = [
      "ajio.com",
      "@ajio",
      "noreply@ajio.com",
    ...
    const isAjio = ajioDomains.some((domain) => from.includes(domain));
    console.log(`🎯 AJIO: Is from AJIO domain? ${isAjio} (${from})`);

    if (!isAjio) {
      console.log("❌ AJIO: Not from AJIO domain");
      return false;
    }
    ...
    console.log(`📧 AJIO: Has order keywords? ${hasOrderKeyword} (${subject})`);
    ...
    console.log(`🚫 AJIO: Is promotional? ${isPromotional}`);

    const canParse = isAjio && hasOrderKeyword && !isPromotional;
    console.log(`✅ AJIO: Final canParse result: ${canParse}`);

    return canParse;
  }

  /**
   * Enhanced parse method for AJIO emails
   */
  parse(emailData) {
    console.log("🛍️ AJIO: Starting enhanced parsing...");
    console.log(`📧 Subject: ${emailData.subject}`);
    console.log(`📧 From: ${emailData.from}`);

    try {
      // Step 1: Clean HTML content
      const cleanContent = this.cleanAjioHtml(
        emailData.html || emailData.text || ""
      );
      console.log("✅ AJIO: HTML cleaned successfully");

      // Step 2: Detect email type
      const emailType = this.detectAjioEmailType(
        emailData.subject,
        cleanContent
      );
      console.log(`📧 AJIO: Email type detected - ${emailType}`);
      ...
      if (!orderId) {
        console.log("❌ AJIO: No order ID found - cannot process");
        return null;
      }
      console.log(`✅ AJIO: Order ID extracted - ${orderId}`);

      // Step 4: Extract amount
      const amount = this.extractOrderAmountRobust(cleanContent, emailType);
      console.log(`💰 AJIO: Amount extracted - ₹${amount || "not found"}`);
      ...
      console.log(`📦 AJIO: Products found - ${products.length} items`);
      ...
      const metadata = this.extractAjioMetadata(cleanContent, emailData);
      ...
      console.log("📊 AJIO: Enhanced parsing result:", {
      ...
      console.error("❌ AJIO enhanced parser error:", error);
      return null;
    }
  }

  /**
   * Clean AJIO HTML with comprehensive fixes
   */
  cleanAjioHtml(htmlContent) {
    if (!htmlContent) return "";

    console.log("🧹 Cleaning AJIO HTML...");
  ...
  extractOrderIdRobust(content, subject) {
    console.log("🆔 AJIO: Robust order ID extraction...");

    const orderIdPatterns = [
      // AJIO specific patterns
      /Order\s*(?:ID|Number|#)\s*[:\-]?\s*([A-Z0-9]{6,20})/gi,
      /(?:Order|ID)[:\s]*([A-Z0-9]{6,20})/gi,
      /([A-Z]{2}\d{8,15})/gi, // Format like AJ123456789
    ...
    if (subject) {
      ...
      for (const pattern of orderIdPatterns) {
        ...
        for (const match of matches) {
          ...
          if (this.isValidAjioOrderId(orderId)) {
            console.log(`✅ AJIO: Order ID from subject - ${orderId}`);
            return orderId;
          }
        }
      }
    ...
    for (const pattern of orderIdPatterns) {
      ...
      for (const match of matches) {
        const orderId = match[1];
        if (this.isValidAjioOrderId(orderId)) {
          console.log(`✅ AJIO: Order ID from content - ${orderId}`);
          return orderId;
        }
      }
    }

    console.log("❌ AJIO: No valid order ID found");
    return null;
  }

  /**
   * Validate AJIO order ID
   */
  isValidAjioOrderId(orderId) {
    if (!orderId) return false;

    // Must be reasonable length
    if (orderId.length < 6 || orderId.length > 25) {
      console.log(`❌ AJIO: Invalid order ID length - ${orderId}`);
      return false;
    }
    ...
    if (isInvalid) {
      console.log(`❌ AJIO: Invalid order ID pattern - ${orderId}`);
      return false;
    }

    console.log(`✅ AJIO: Valid order ID - ${orderId}`);
    return true;
  }
  ...
  extractOrderAmountRobust(content, emailType) {
    console.log("💰 AJIO: Robust amount extraction...");

    const amountPatterns = [
      // High priority patterns
      /(?:Total Amount|Grand Total|Order Total|Amount Paid)[:\s]*₹\s*([\d,]+(?:\.\d{2})?)/gi,
      /(?:Final Amount|Bill Total)[:\s]*₹\s*([\d,]+(?:\.\d{2})?)/gi,
    ...
    if (foundAmounts.length > 0) {
      ...
      console.log(`✅ AJIO: Selected amount ₹${selectedAmount}`);
      return selectedAmount;
    }

    console.log("❌ AJIO: No valid amount found");
    return null;
  }
  ...
  getAmountPriority(matchText) {
    ...
    if (text.includes("total amount") || text.includes("grand total"))
      return 100;
    if (text.includes("order total") || text.includes("amount paid")) return 90;
    if (text.includes("final amount") || text.includes("bill total")) return 80;
  ...
  extractProductsRobust(content, emailType, orderId, subject, totalAmount) {
    console.log("📦 AJIO: Robust product extraction...");

    // Try to extract actual products
    const extractedProducts = this.extractActualAjioProducts(content);

    if (extractedProducts.length > 0) {
      console.log(`✅ AJIO: Found ${extractedProducts.length} actual products`);
    ...
    if (subjectProduct) {
      console.log("✅ AJIO: Extracted product from subject");
      return [
        {
          name: subjectProduct,
    ...
    // Fallback product
    console.log("📦 AJIO: Creating fallback product...");
    const productName = `AJIO ${this.getEmailTypeLabel(
      emailType
    )} Order ${orderId}`;
   ...
   */
  extractActualAjioProducts(content) {
    const products = [];
    ...
    for (const pattern of productPatterns) {
      ...
      for (const match of matches) {
        ...
        if (this.isValidAjioProduct(productName)) {
          products.push({
  ...
  /**
   * Validate AJIO product
   */
  isValidAjioProduct(name) {
    if (!name || name.length < 5) return false;

    const garbagePatterns = [
      /order|total|amount|ajio|email|notification/i,
  ...
  /**
   * Detect AJIO email type
   */
  detectAjioEmailType(subject, content) {
  ...
  /**
   * Extract AJIO metadata
   */
  extractAjioMetadata(content, emailData) {
    const metadata = {};
  ...
  calculateEnhancedConfidence(orderId, amount, products, emailType, metadata) {
    ...
    if (metadata.trackingId || metadata.expectedDelivery) confidence += 0.05;

    return Math.round(Math.min(confidence, 1.0) * 100) / 100;
  }
}

module.exports = AjioParser;
