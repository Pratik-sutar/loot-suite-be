// src/services/parsers/tatacliqParser.js
...
class TataCliqParser {
  constructor() {
    this.platform = "tatacliq";
  }
  ...
  static canParse(emailData) {
    ...
    const isTataCliq =
      from.includes("tatacliq.com") ||
      from.includes("@tatacliq") ||
      from.includes("tata cliq");
    if (!isTataCliq) return false;
  ...
  parse(emailData) {
    try {
      const cleanContent = this.cleanTataCliqHtml(
      ...
      console.error("❌ TATACLIQ parser error:", error);
      return null;
    }
  }

  cleanTataCliqHtml(htmlContent) {
    return htmlContent
      .replace(/&nbsp;/g, " ")
      .replace(/&amp;/g, "&")
      .replace(/&#8377;/g, "₹")
  ...
  extractOrderId(content, subject) {
    const patterns = [
      /Order\s*(?:ID|Number|#)\s*[:\-]?\s*(TC[A-Z0-9]{8,15})/gi,
  ...
  extractProducts(content, emailType, orderId) {
    ...
    if (products.length === 0) {
      products.push({
        name: `Tata CLiQ ${this.getEmailTypeLabel(emailType)} ${orderId}`,
        quantity: 1,
        price: 0,
  ...
  isValidProduct(name) {
    ...
    const invalidPatterns = [
      /order|total|amount|tata|cliq|email|notification/i,
    ];
