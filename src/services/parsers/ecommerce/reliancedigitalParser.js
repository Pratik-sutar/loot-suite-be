// src/services/parsers/reliancedigitalParser.js
...
class RelianceDigitalParser {
  ...
  static canParse(emailData) {
    ...
    const isRelianceDigital =
      from.includes("reliancedigital.in") ||
      from.includes("@reliancedigital") ||
      from.includes("reliance digital");
    if (!isRelianceDigital) return false;
  ...
  parse(emailData) {
    try {
      const cleanContent = this.cleanRelianceDigitalHtml(
  ...
  cleanRelianceDigitalHtml(htmlContent) {
    return htmlContent
      .replace(/&nbsp;/g, " ")
      .replace(/&amp;/g, "&")
      .replace(/&#8377;/g, "₹")
  ...
  extractOrderId(content, subject) {
    const patterns = [
      /Order\s*(?:ID|Number|#)\s*[:\-]?\s*(RD[A-Z0-9]{8,15})/gi,
  ...
  extractProducts(content, emailType, orderId) {
    ...
    if (products.length === 0) {
      products.push({
        name: `Reliance Digital Electronics Order ${orderId}`,
        quantity: 1,
        price: 0,
  ...
  isValidProduct(name) {
    ...
    const invalidPatterns = [
      /order|total|amount|reliance|digital|email|notification/i,
    ];
...
module.exports = RelianceDigitalParser;
