// src/services/parsers/firstcryParser.js
...
class FirstCryParser {
  constructor() {
    this.platform = "firstcry";
  }
  ...
  static canParse(emailData) {
    ...
    const isFirstCry =
      from.includes("firstcry.com") || from.includes("@firstcry");
    if (!isFirstCry) return false;
  ...
  parse(emailData) {
    try {
      const cleanContent = this.cleanFirstCryHtml(
  ...
  extractOrderId(content, subject) {
    const patterns = [
      /Order\s*(?:ID|Number|#)\s*[:\-]?\s*(FC[A-Z0-9]{8,15})/gi,
  ...
  extractAmount(content, emailType) {
    const patterns = [
      /(?:Total Amount|Grand Total|Order Total|Amount Paid|Bill Total)[:\s]*₹\s*([\d,]+(?:\.\d{2})?)/gi,
      /₹\s*([\d,]+(?:\.\d{2})?)/g,
    ];
  ...
  extractProducts(content, emailType, orderId) {
    ...
    const productPatterns = [
      ...
      /Item[:\s]*([A-Z][a-zA-Z0-9\s\-&.'()]{8,80})/gi,
    ];
    ...
    if (products.length === 0) {
      products.push({
        name: `FirstCry Baby/Kids Order ${orderId}`,
        quantity: 1,
        price: 0,
  ...
  isValidProduct(name) {
    ...
    const invalidPatterns = [/order|total|amount|firstcry|email|notification/i];
