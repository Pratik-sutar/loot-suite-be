// src/services/parsers/ekartParser.js
...
class EkartParser {
  ...
  static canParse(emailData) {
    ...
    const isEkart =
      from.includes("ekart.in") ||
      from.includes("@ekart") ||
      from.includes("ekart");
    if (!isEkart) return false;
  ...
  parse(emailData) {
    try {
      const cleanContent = this.cleanEkartHtml(
      ...
      return {
        ...
        amount: 0,
        formattedAmount: "Flipkart Logistics",
        products: [
          {
            name: `${
            ...
            quantity: 1,
            price: 0,
            formattedPrice: "Flipkart Logistics",
            trackingId: trackingId,
            carrier: "Ekart",
          },
        ],
  ...
  getEmailTypeLabel(emailType) {
    const labels = {
      ...
      courier_notification: "Flipkart Logistics",
    };
