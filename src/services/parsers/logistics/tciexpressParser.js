// src/services/parsers/tciexpressParser.js
const { cleanHtml, extractTextContent } = require("../../../utils/htmlCleaner");

class TciExpressParser {
  constructor() {
    this.platform = "tciexpress";
  }
  ...
  static canParse(emailData) {
    ...
    const isTciExpress =
      from.includes("tciexpress.in") ||
      from.includes("@tciexpress") ||
      from.includes("tci express");
    if (!isTciExpress) return false;
  ...
  parse(emailData) {
    try {
      const cleanContent = this.cleanTciExpressHtml(
      ...
      return {
        ...
        amount: 0,
        formattedAmount: "Freight Service",
        products: [
          {
            name: `${
              shipmentInfo.description || "Freight Package"
            } - ${this.getEmailTypeLabel(emailType)}`,
            quantity: 1,
            price: 0,
            formattedPrice: "Freight Service",
            trackingId: trackingId,
            carrier: "TCI Express",
          },
        ],
  ...
  cleanTciExpressHtml(htmlContent) {
    return htmlContent
      .replace(/&nbsp;/g, " ")
      .replace(/&amp;/g, "&")
  ...
  extractTrackingId(content, subject) {
    const patterns = [
      /Consignment[:\s]*(?:ID|Number)[:\s]*([A-Z0-9]{8,20})/gi,
  ...
  getEmailTypeLabel(emailType) {
    const labels = {
      shipping_notification: "Freight Shipment",
      delivery_notification: "Delivered Package",
      tracking_update: "Shipment Update",
      courier_notification: "Freight Service",
    };
    return labels[emailType] || "Freight Package";
  }
...
module.exports = TciExpressParser;
