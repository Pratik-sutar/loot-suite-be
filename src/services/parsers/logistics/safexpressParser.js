// src/services/parsers/safexpressParser.js
const { cleanHtml, extractTextContent } = require("../../../utils/htmlCleaner");

class SafexpressParser {
  constructor() {
    this.platform = "safexpress";
  }
  ...
  static canParse(emailData) {
    ...
    const isSafexpress =
      from.includes("safexpress.com") ||
      from.includes("@safexpress") ||
      from.includes("safexpress");
    if (!isSafexpress) return false;
  ...
  parse(emailData) {
    try {
      const cleanContent = this.cleanSafexpressHtml(
  ...
  cleanSafexpressHtml(htmlContent) {
    return htmlContent
      .replace(/&nbsp;/g, " ")
      .replace(/&amp;/g, "&")
...
module.exports = SafexpressParser;
