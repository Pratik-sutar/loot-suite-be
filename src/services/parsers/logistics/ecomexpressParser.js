// src/services/parsers/ecomexpressParser.js
const { cleanHtml, extractTextContent } = require("../../../utils/htmlCleaner");

class EcomExpressParser {
  constructor() {
    this.platform = "ecomexpress";
  }
  ...
  static canParse(emailData) {
    ...
    const isEcomExpress =
      from.includes("ecomexpress.in") ||
      from.includes("@ecomexpress") ||
      from.includes("ecom express");
    if (!isEcomExpress) return false;
  ...
  parse(emailData) {
    try {
      const cleanContent = this.cleanEcomExpressHtml(
  ...
  cleanEcomExpressHtml(htmlContent) {
    return htmlContent
      .replace(/&nbsp;/g, " ")
      .replace(/&amp;/g, "&")
  ...
  extractShipmentInfo(content) {
    ...
    const originPatterns = [/(?:Origin|From)[:\s]*([^,\n]{5,100})/gi];
...
module.exports = EcomExpressParser;
