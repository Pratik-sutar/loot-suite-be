// src/services/parsers/gatiParser.js
...
class GatiParser {
  ...
  static canParse(emailData) {
    ...
    const isGati =
      from.includes("gati.com") ||
      from.includes("@gati") ||
      from.includes("gati");
    if (!isGati) return false;
  ...
  parse(emailData) {
    try {
      const cleanContent = this.cleanGatiHtml(
  ...
  extractShipmentInfo(content) {
    ...
    const originPatterns = [/(?:Origin|From|Consignor)[:\s]*([^,\n]{5,100})/gi];
